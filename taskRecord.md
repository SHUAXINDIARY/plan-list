...
根据现有代码风格完善 Oxfmt 格式化规则。

## 完成过程

1. 查询 Oxfmt 官方快速开始、配置说明和配置字段参考；Context7 因本地 npm 缓存 I/O 问题无法执行，改用用户提供的官方文档页面核对字段。
2. 检查仓库主源码的缩进、引号、分号、尾逗号和 JSX 换行风格，并核对已安装的 Oxfmt 0.66.0 schema。
3. 固定四空格、100 列、双引号、分号、尾逗号、LF、对象换行与导入排序规则。
4. 排除构建输出和两个由构建流程维护的 TypeScript 生成清单，避免格式化命令改写生成内容。
5. 使用 `pnpm exec oxfmt --check .oxfmtrc.json` 验证配置文件格式通过，并执行 `git diff --check`。

## 修改具体文件

- `.oxfmtrc.json`：新增项目统一格式化规则和生成文件忽略模式。
- `taskRecord.md`：追加本次 Oxfmt 配置记录。
