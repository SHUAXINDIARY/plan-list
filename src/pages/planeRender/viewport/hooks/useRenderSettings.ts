...
export const useRenderSettings = (): AircraftRenderSettingsController => {
    const [settings, setSettings] = useState<AircraftRenderSettings>(
        createDefaultRenderSettings,
    );
    ...
    const onEnvironmentPresetChange = (
        event: ChangeEvent<HTMLSelectElement>,
    ): void => {
        const value = event.currentTarget.value;
    ...
    /** 更新环境反射强度。 */
    const onEnvironmentIntensityChange = (
        event: ChangeEvent<HTMLInputElement>,
    ): void => {
    ...
    /** 更新主方向光强度并切换到自定义照明。 */
    const onKeyLightIntensityChange = (
        event: ChangeEvent<HTMLInputElement>,
    ): void => {
    ...
    /** 更新补光强度并切换到自定义照明。 */
    const onFillLightIntensityChange = (
        event: ChangeEvent<HTMLInputElement>,
    ): void => {
        updateLightingSetting(
            "fillLightIntensity",
    ...
    /** 更新轮廓光强度并切换到自定义照明。 */
    const onRimLightIntensityChange = (
        event: ChangeEvent<HTMLInputElement>,
    ): void => {
        updateLightingSetting(
            "rimLightIntensity",
