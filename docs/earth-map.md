...
2. 将三角形的最长边限制在约 6 度弧长以内，细分后的顶点用重心插值再归一化回球面。

第二项非常重要。直接把大三角形的三个边界点连成平面，面片中央会落入底球内部并被深度缓冲遮挡，视觉上表现为大陆内的深色三角形空白。细分后，所有填充面持续贴合球面。
...
- 调整填充精度：修改 `LANDMASS_FILL_MAX_ARC`。角度更小会减少曲率误差，但会增加顶点数和初始化成本。
